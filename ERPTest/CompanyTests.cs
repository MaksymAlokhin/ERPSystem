...
namespace ERPTest
{
    ...
    {
        protected CompanyTests(DbContextOptions<ApplicationDbContext> contextOptions)
        {
        ...
        public async Task Company_IndexModel_OnGetAsync_CompaniesAreReturned()
        {
            using (var context = new ApplicationDbContext(ContextOptions))
            {
                ...
                Assert.Equal(
                    expectedCompanies.OrderBy(m => m.Name).Take(7).Select(m => m.Name),
        ...
        public async Task Company_IndexModel_OnGetAsync_CompaniesAreReturnedInDescendingOrder()
        {
            using (var context = new ApplicationDbContext(ContextOptions))
            {
                ...
                var actualCompanies = Assert.IsAssignableFrom<List<Company>>(pageModel.Company);
                Assert.Equal(
                    expectedCompanies.OrderByDescending(m => m.Name).Take(7).Select(m => m.Name),
