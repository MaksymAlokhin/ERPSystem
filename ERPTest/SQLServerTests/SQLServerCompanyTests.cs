...
namespace ERPTest
{
    public class SQLServerCompanyTests : IDisposable
    {
        public SQLServerCompanyTests()
        {
            PageSize = 7;
            Connection = new SqlConnection(@"Server=(localdb)\mssqllocaldb;Database=CompanyTests;Trusted_Connection=True;MultipleActiveResultSets=true");
            context = new ApplicationDbContext(new DbContextOptionsBuilder<ApplicationDbContext>().UseSqlServer(Connection).Options);
